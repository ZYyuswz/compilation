# homework 4 Multiplication Table
...
```
LC0:
        .string "%d*%d=%d  "    ; 格式化字符串，用于打印乘法表达式

mul:
        push    rbp             ; 保存旧的基址指针
        mov     rbp, rsp        ; 设置新的基址指针
        sub     rsp, 16         ; 在栈上分配16字节空间用于局部变量
        
        mov     DWORD PTR [rbp-4], 9    ; 外层循环计数器 i = 9 (从9递减到1)
        jmp     .L2             ; 跳转到外层循环条件判断

.L5:                            ; 外层循环体开始
        mov     DWORD PTR [rbp-8], 1    ; 内层循环计数器 j = 1 (从1递增到i)
        jmp     .L3             ; 跳转到内层循环条件判断

.L4:                            ; 内层循环体开始
        ; 准备调用printf的参数
        mov     eax, DWORD PTR [rbp-8]  ; eax = j
        imul    eax, DWORD PTR [rbp-4]  ; eax = i * j (计算结果)
        
        ; 按照System V AMD64 ABI调用约定设置参数
        mov     ecx, eax                ; 第4个参数：计算结果
        mov     edx, DWORD PTR [rbp-8]  ; 第3个参数：j
        mov     esi, DWORD PTR [rbp-4]  ; 第2个参数：i
        mov     edi, OFFSET FLAT:LC0    ; 第1个参数：格式字符串地址
        mov     eax, 0                  ; 对于printf，eax表示使用的向量寄存器数量(这里为0)
        call    printf                  ; 调用printf函数
        
        add     DWORD PTR [rbp-8], 1    ; j++ (内层循环计数器递增)
...
.L3:                            ; 内层循环条件判断
        ...
        ; 打印换行符(这里应该有相应的代码，但在片段中被省略了)
        ; mov edi, 10
        ; call putchar
        
        sub     DWORD PTR [rbp-4], 1    ; i-- (外层循环计数器递减)
...
.L2:                            ; 外层循环条件判断
        ...
        nop                     ; 空操作(用于对齐或调试)
        nop
        leave                   ; 恢复栈指针(相当于 mov rsp, rbp; pop rbp)
        ret                     ; 函数返回
```
...
![](source_image/hw404.png)

对应部分反汇编代码及注释为：

```
.LC1:
        .string "x y "                    ; 错误报告的表头字符串
.LC2:
        .string "%d %d  error\n"          ; 错误信息的格式化字符串
...
err:
        push    rbp                       ; 保存旧的基址指针
        mov     rbp, rsp                  ; 设置新的基址指针
        sub     rsp, 32                   ; 在栈上分配32字节空间
        
        mov     QWORD PTR [rbp-24], rdi   ; 保存第一个参数（二维数组指针）到栈上 [rbp-24]
        
        mov     edi, OFFSET FLAT:.LC1     ; 准备puts函数的参数：字符串"x y "
        call    puts                      ; 打印表头
        
        mov     DWORD PTR [rbp-4], 1      ; 外层循环计数器 i = 1 [rbp-4]
        jmp     .L2                       ; 跳转到外层循环条件判断

.L6:                                      ; 外层循环体开始
        mov     DWORD PTR [rbp-8], 1      ; 内层循环计数器 j = 1 [rbp-8]
        jmp     .L3                       ; 跳转到内层循环条件判断
...
.L5:                                      ; 内层循环体开始
        ...
        mov     ecx, eax                  ; ecx = 期望结果 (i * j)
        
        ; 计算数组元素地址：arr[i-1][j-1]
        ; 数组结构：9x9的二维数组，每个元素4字节
        mov     eax, DWORD PTR [rbp-4]    ; eax = i
        movsx   rdx, eax                  ; 符号扩展 i 到64位
        mov     rax, rdx                  ; rax = i
        sal     rax, 3                    ; rax = i * 8
        add     rax, rdx                  ; rax = i * 8 + i = i * 9
        sal     rax, 2                    ; rax = i * 9 * 4 = i * 36
        lea     rdx, [rax-36]             ; rdx = (i * 36) - 36 = (i-1) * 36
        ; 此时 rdx = (i-1) * 9 * 4 = (i-1)行的起始字节偏移
        
        mov     rax, QWORD PTR [rbp-24]   ; rax = 数组首地址
        add     rdx, rax                  ; rdx = 数组首地址 + 行偏移
        
        mov     eax, DWORD PTR [rbp-8]    ; eax = j
        sub     eax, 1                    ; eax = j - 1
        cdqe                              ; 符号扩展到64位
        mov     eax, DWORD PTR [rdx+rax*4] ; eax = arr[i-1][j-1]
        ; 此时 eax 包含从数组读取的实际值
        
        ; 比较期望值和实际值
        cmp     ecx, eax                  ; 比较 (i*j) 和 arr[i-1][j-1]
        je      .L4                       ; 如果相等，跳过错误报告
        
        ; 报告错误：打印错误信息
        mov     edx, DWORD PTR [rbp-8]    ; 第三个参数：j
        mov     eax, DWORD PTR [rbp-4]    ; 第二个参数：i
        mov     esi, eax                  ; esi = i
        mov     edi, OFFSET FLAT:.LC2     ; 第一个参数：格式化字符串
        mov     eax, 0                    ; 对于printf，eax=0表示没有向量参数
        call    printf                    ; 调用printf打印错误信息
...
.L3:                                      ; 内层循环条件判断
        ...
        add     DWORD PTR [rbp-4], 1      ; i++ (外层循环计数器递增)
